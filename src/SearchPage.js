...
class SearchPage extends Component {
    ...
    render() {
        const { searchResults, onSearchBooks, switchShelf } = this.props
     ...
     if (query) {
         ...
         showingResults = searchResults.filter(result => match.test(result.title))
            ...
            <div className="search-books-results">
              <ol className="books-grid">
                { showingResults.map(book => (
                  <li key={book.id}>
                    <Book book={book} changeShelf={switchShelf} />
                  </li>
                ))}
              </ol>
              <p>{errorMessage}</p>
